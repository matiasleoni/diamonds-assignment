...
def pricing(carat, color, clarity):
    ...
    Parameters:
        ...
        color: The color of the diamond. Should be a single letter from 'D' to 'J'.
    ...
    """
    control = True
    # Check if carat is float and positive
    try:
        carat = float(carat)
        if carat <= 0:
            control = False
            error_msg = 'Error: You must introduce a positive number for carat. Please try again.'    
    except ValueError:
        control = False
        error_msg = 'Error: You must introduce a number for carat. Please try again.'    
    # Check color is in list
    if color.upper() not in list('DEFGHIJ'):
        control = False
        error_msg = f'Error: {color} is not a correct value for color. Please use a letter from D to J.'
    else:
        color = color.upper()
    # Check clarity is in list
    if clarity.upper() not in ["I1", "IF", "SI1", "SI2", "VS1", "VS2", "VVS1", "VVS2"]:
        control = False
        error_msg = f'Error: {clarity} is not a correct value for clarity. Please use I1, IF, SI1, SI2, VS1, VS2, VVS1, VVS2.'
    else:
